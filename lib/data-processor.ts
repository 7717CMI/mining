  ...
  const filtered = data.filter((record) => {
    ...
    if (!geoMatch) {
      ...
      const regionToCountriesMap: Record<string, string[]> = geographyCountries || {
        'West Africa': ['Guinea', 'Ghana', 'Benin', 'Ivory Coast (Côte d\'Ivoire)', 'Mauritania', 'Senegal', 'Nigeria', 'Niger', 'Togo', 'Burkina Faso']
      }
      ...
      if (record.is_aggregated === true) {
        ...
        if (selectedLevel1Segments.length > 0) {
          if (isRegionalSegmentType) {
            const regionalGeographies = ['Morocco', 'West Africa', 'Global']
